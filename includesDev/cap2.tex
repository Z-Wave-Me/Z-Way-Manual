...
of \zway binary distributions for the various platforms supported:
	...
	\item Popp:  For Popp Hub, Mediatec CPU openWRT
...
\item Reset button
\item Open hole for a PigTail antenna. You need to break off the PCBA antenna to make this work.
...
\begin{itemize}
\item Australia/New Zealand/Brazil/Peru (ANZ) [default]
\item Hongkong (HK)
...
\murl{http://www.zwave.me/fileadmin/download/changezwf.sh}. Just execute the script with
...
\cmdline{/dev/ttyUSB0} or \cmdline{/dev/cu.usbmodem} or similar.
...
versions, one with a license and one without), the license can be loaded once \zway is 
...
However, losing the UZB key means losing the license!
\end{itemize}
...
using devices of different \zwave networks together.
...
\item There is a shell script available at \murl{http://www.zwave.me/fileadmin/download/changezwf.sh} 
...
\item min. 16 MB Flash memory and 16 MB RAM,
...
\begin{quote}
\textbf{http://razberry.zwave.me/index.php?id=28}.
\end{quote}
