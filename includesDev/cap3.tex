...
\begin{itemize}
\item bind(ip, port) or bind(port) binds socket to port (integer number). ip should be a string like "192.168.0.1". If omited "127.0.0.1" is used.
...
\item onrecv(data, host, port) called on new data receiption from remote side. "this" object will be defined like in onconnect.
\item onconnect(host, port) called once connection to remote side is established or on new connection accept. For UDP sockets and connected TCP sockets "this" object reffers to the socket itself, while for accepted TCP sockets "this" reffers to the client individual object.
\item onclose(host, port) called on socket close by remote or due to close() call. Note that for TCP sockets this callback is called for client sockets on connection close and for binded listening socket if close() is called. "this" object will be defined like in onconnect.
\end{itemize}
